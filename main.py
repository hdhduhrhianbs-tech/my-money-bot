...
app = Flask('')
@app.route('/')
def home(): return "Vizitka is Online"
def run(): app.run(host='0.0.0.0', port=8080)
def keep_alive():
    t = Thread(target=run)
    t.start()

# ДАННЫЕ (ТВОЙ ТОКЕН И ТВОЙ ID)
TOKEN = '8682627312:AAHcQ2-cY-wuyxPHEMr0jT4kATwMqwfxZco'
ADMIN_ID = 6863105636 
MY_USERNAME = 'MuichiroHGP' # Твой ник в ТГ без собаки
...
def handle_text(message):
    if message.text == "💰 Прайс та послуги":
        ...
        # ХИТРОСТЬ: tg://resolve сразу открывает приложение
        btn = types.InlineKeyboardButton("Написати особисто 💬", url=f"tg://resolve?domain={MY_USERNAME}")
        markup.add(btn)
        bot.send_message(message.chat.id, "Тисніть на кнопку, щоб перейти в чат зі мною:", reply_markup=markup)
...
def get_bot_type(message):
    ...
    bot.send_message(message.chat.id, "Коротко опишіть вашу ідею:")
...
def get_contact(message, bot_type, biz_desc):
    ...
    bot.send_message(message.chat.id, "✅ Дякую! Я отримав вашу заявку і скоро напишу вам.", reply_markup=main_menu())
